...
func TestDefaultConfig(t *testing.T) {
	...
	if config.Version != SchemaVersion || len(config.Tags) != 1 || config.Tags[0] != "general" || len(config.PromptTags) != 1 || config.PromptTags[0] != "general" || len(config.Defaults.Tags) != 1 || len(config.Defaults.PromptTags) != 1 || len(config.Defaults.Agents) != 2 {
