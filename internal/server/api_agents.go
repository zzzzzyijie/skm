...
var managedAgents = []domain.Agent{
	...
	domain.AgentOpenCode, domain.AgentTrae, domain.AgentHermes,
	domain.AgentOpenClaw,
}
...
func (s *Server) handleListAgents(w http.ResponseWriter, r *http.Request) {
	...
	for _, agent := range managedAgents {
		...
		if agent.Valid() {
			...
			descriptor.Note = "No documented Agent Skills directory or SKILL.md loader"
		}
...
func (s *Server) handleSaveCustomAgent(w http.ResponseWriter, r *http.Request) {
	var definition domain.AgentDefinition
	if err := readJSON(r, &definition); err != nil {
		writeError(w, http.StatusBadRequest, err)
		return
	}
	definition.ID = domain.Agent(strings.ToLower(strings.TrimSpace(string(definition.ID))))
	definition.Name = strings.TrimSpace(definition.Name)
	definition.SkillsPath = strings.TrimSuffix(strings.TrimSpace(definition.SkillsPath), "/")
	...
	err := s.withLock(func() error {
		...
		for i, existing := range config.Agents {
			if existing.ID != definition.ID {
				continue
			}
			if existing.SkillsPath != definition.SkillsPath {
				active, err := s.agentIsActive(definition.ID)
				if err != nil {
					return err
				}
				if active {
...
func (s *Server) handleDeleteCustomAgent(w http.ResponseWriter, r *http.Request) {
	id := domain.Agent(r.PathValue("id"))
	err := s.withLock(func() error {
		config, err := s.store.LoadConfig()
		if err != nil {
			return err
		...
		if !found {
			return fmt.Errorf("custom agent %q not found", id)
		}
		active, err := s.agentIsActive(id)
		if err != nil {
			return err
		}
		if active {
			return fmt.Errorf("disable all Skills for %s before deleting it", id)
		}
		config.Agents = kept
		config.Defaults.Agents = configuredAgents(config.Defaults.Agents, config.Agents)
		return s.store.SaveConfig(config)
...
func (s *Server) agentIsActive(agent domain.Agent) (bool, error) {
	state, err := s.store.LoadState()
	if err != nil {
		return false, err
	}
