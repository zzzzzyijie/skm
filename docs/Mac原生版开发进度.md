...
- [x] **【Skills / Prompts 详情】移除大标题前置图标**：
  - 现状：`SkillsViews.swift`（`square.stack.3d.up.fill`）与 `PromptsViews.swift`（`text.bubble.fill`）详情页顶部大标题前方带有 48x48 的彩色背景大图标；
  - 完成：移除 Skill 与 Prompt 详情标题前的 48x48 彩色图标，标题直接与健康状态对齐，详情页顶部更简洁。
- [x] **【项目导入 Skill 弹窗】文案修正**：
  - 现状：`ProjectSkillImportSheet.swift` 底部操作栏展示“将导入到 %@”（当当前项目名为 skm 时显示为“将导入到 skm”）；
  - 完成：底部操作栏统一显示「导入到项目」，不再重复项目名导致“导入回全局库”的歧义。
- [x] **【设置】左侧导航栏图标对齐优化**：
  - 现状：`SettingsView` 侧边栏使用原生 `Label`，因各 SF Symbols（齿轮、文件夹、双人、箱子、循环箭头等）固有宽度不一，导致图标未居中、文字左边缘不齐整；
  - 完成：将设置项改为固定 20pt 宽度的 SF Symbol 居中容器与文本组合，统一图标中心线和文字起始位置。
- [x] **【Projects 列表】调整内容左右边距**：
  - 现状：精简移除图标与右侧 Badge 后，项目卡片文本紧贴列表单元格最左侧，在选中高亮态下显得拥挤局促；
  - 完成：为项目名称与路径内容增加 10pt 对称水平留白，选中高亮内的文本不再紧贴边缘。
- [x] **【顶部工具栏】优化「图标和文本」模式下的垂直间距与对称性**：
  - 现状：当 macOS 工具栏切换为【图标和文本】（Icon and Text）展示模式时，系统按钮的图标与文本上下间距局促、垂直分布不够对称；
  - 完成：新增通用 `topToolbarActionStyle`，为主窗口列表与详情工具栏按钮统一 regular 控件尺寸、borderless 样式及 2pt 对称垂直留白，同时保留原生 `Label` 对“仅图标 / 图标和文本”模式的自适应。
- [x] **【存到我的 Skill 弹窗】「跟随项目」模式补充说明文案**：
  - 现状：`ProjectsViews.swift` 中的 `ProjectMigrationSheet` 弹窗在选择“复制到 Library”时展示了选项与说明，但切换为“跟随项目”（`symlink` 模式）时下方无任何说明；
  - 完成：「跟随项目」下补充软链接及项目修改实时同步说明，并在模式切换时保持与复制选项的信息对称。
- [x] **【Skills 详情】移除版本哈希值与字符数统计**：
  - 现状：`SkillsViews.swift` 详情元信息行展示了版本前缀哈希（`skill.hash.prefix(8)`）及正文字符数（`\(body.count) 字符`）；
  - 完成：移除 Skill 详情的哈希前缀与正文字符数，元信息区只保留来源。
- [x] **【Prompts 详情】移除大小与字符数统计**：
  - 现状：`PromptsViews.swift` 详情元信息行展示了 `\(body.count) 字符` 统计；
  - 完成：移除 Prompt 详情的正文字符数，元信息区仅保留来源及标签。
