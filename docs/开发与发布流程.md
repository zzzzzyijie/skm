...
3. Agent 管理能否重新扫描设备、区分已检测和其他支持项、自由增删 Claude Code/Codex，并正常添加或编辑自定义 Agent；选项内不存在按钮嵌套或点击串扰。
...
推送 Tag 后，GitHub Actions 会自动执行 Go/Swift/XCUITest、构建 macOS/Linux 的 CLI 归档、
签名并公证 Universal 2 原生 App、生成 `checksums.txt`、创建 GitHub Release，并更新 Homebrew
Formula。确认 Release 工作流成功，且 Release 页面同时包含 CLI 归档、`SKM-<version>-universal.zip`、
`SKM-<version>-universal.dmg` 和两类校验文件。原生 App 所需的五项 Actions Secrets 见
[发布指南](发布指南.md#21-一次性配置原生-app-发布凭据)。
