...
> 状态：0.6.0 Phase 1 至 Phase 3 原型均已落地；支持简体中文与英文，项目清单、历史回滚、Quick Look 和 Sparkle 更新入口已实现；Prompt 变量定义保持兼容，原生 App 不提供“填写变量”入口；正式 Release 待触发
...
- 未配置个人同步时，同步按钮打开 Settings 的“Git 同步”页面。
- 冲突没有全部解决时，“开始同步”不可用。
...
- App、Core 和 Schema 版本；
- `~/.skm` 共享存储说明；
- 已管理 Agent、技能来源和个人 Git 同步状态摘要。

### Agent 管理

- 选择允许 SKM 部署 Skill 的 Agent；
- 添加、编辑和删除自定义 Agent；
- 展示检测状态与实际 Skill 路径。
...
- 添加团队或社区 Git Source；
- 单来源更新、统一同步和删除绑定；
- 展示 ref、revision 和最近一次同步结果。

### Git 同步

- 私人仓库 URL、ref、root 和连接测试；
- 上传、下载、删除与冲突预览；
- 逐项选择保留本地或使用远端；
- 不提供 Token 明文输入和保存。
