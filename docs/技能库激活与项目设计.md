...
skm list [--tag ...]
skm show <skill>
skm update <skill> <skill-path> [--base-hash <hash>]
skm remove <skill>
skm prune [--dry-run]
```
...
独立本地 Skill 使用写时复制更新：完整校验临时副本后，将新内容写入
`objects/<new-hash>/<name>` 并更新 Catalog 指针，绝不原地修改旧对象。用户级 Activation
自动刷新到新快照，项目级固定 Activation 继续引用旧快照。Git Source Skill 和跟随项目的
实时来源不在 Library 中直接编辑。
