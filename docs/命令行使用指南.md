...
- Prompt 独立保存和渲染，不复用 Skill Activation，也不调用模型 API。
...
Prompt 是单个 `PROMPT.md` 文件，YAML frontmatter 定义名称、描述、标签和变量，正文是模板。
变量使用 `{{variable}}`，只做文本替换，不执行脚本或模板函数：
...
包含敏感值时优先使用 `--var-file`；变量值只参与本次输出，不写入 Prompt Catalog。
...
CLI 从一个完整 Skill 目录创建新快照，因此可以同时更新 `SKILL.md`、`scripts/`、
`references/` 和 `assets/`：
...
自动化流程可以携带读取时的 hash，避免覆盖其他编辑：

```bash
skm update local/code-review "$HOME/my-skills/code-review" --base-hash <hash>
```

更新不会原地修改 `~/.skm/objects/`，而是创建新的内容寻址快照。用户级 Activation
会自动切换到新快照；项目级 Activation 继续使用其固定版本。Git Source Skill 必须在
源仓库修改后运行 `source update`，跟随项目的 Skill 应在项目源目录修改或先转为独立副本。

Web 管理界面当前支持编辑完整 `SKILL.md`，其他文件保持不变。
